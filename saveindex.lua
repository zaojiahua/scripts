...
function SaveIndex:Save(callback)
    ...
    local insz, outsz = SavePersistentString(self:GetSaveIndexName(), data, ENCODE_SAVES, callback)
end
...
function SaveIndex:OnGenerateNewWorld(saveslot, savedata, cb)
	...
	local insz, outsz = SavePersistentString(filename, savedata, ENCODE_SAVES, onsavedatasaved)	
end
