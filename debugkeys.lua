...
AddGameDebugKey(KEY_F3, function()
    ...
    if TheInput:IsKeyDown(KEY_SHIFT) then
        --capped, dead
        local days_survived, start_xp, reward_xp, new_xp, capped = 99999, 1600, 0, 1600, true
    ...
    elseif TheInput:IsKeyDown(KEY_ALT) then
        -- not capped, dead
        local days_survived, start_xp, reward_xp, new_xp, capped = 99999, 0, 1600, 1600, false
        TheFrontEnd:PushScreen(DeathScreen(days_survived, start_xp, nil, capped))
...
local function SetPlayerPosition( x, z)
