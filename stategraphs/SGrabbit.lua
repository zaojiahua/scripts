...
{
	-- ActionHandler类.__call( action, state, condition = nil);
    ActionHandler(ACTIONS.EAT, "eat"),-- 当主角发出ACTIONS.EAT动作时，就进入"eat"状态，表现出来就是开始吃的动作了。
...
local events=
{
    CommonHandlers.OnSleep(),-- 处理gosleep事件
    CommonHandlers.OnFreeze(),-- 处理freeze时间
	-- 处理attacked事件
